...
<x-layout rightPanel="hidden">
    ...
    <div class="grid grid-cols-1 gap-y-1">
        ...
        <section>
            ...
            <div class="text-gray-400 flex space-x-3">
                ...
                <a href="{{ route('friend-request.index', [Auth::user()->id]) }}"><span class="text-white font-semibold">{{ Auth::user()->friendRequestsSent->count() }}</span> Followers</a>
            </div>
        </section>
        @else
        <section>
            ...
            <section class="flex justify-end space-x-2 {{  ($request && $request->receiver_id) === Auth::user()->id ? 'invisible' : '' }}">
                ...
                @elseif ($request && $request->status === 'accepted')
            ...
            <div class="text-gray-400 flex space-x-3">
                <p><span class="text-white font-semibold">{{ $user->friends()->count() }}</span> Friends</p>
                <p><span class="text-white font-semibold">{{ $user->friendRequestsSent->count() }}</span> Followers</p>
            </div>
        </section>
        @endif
    ...
    <div class="my-5">
        ...
        <nav class="grid {{ $ownProfile ? 'grid-cols-4' : 'grid-cols-2' }} text-center">
            @if (Auth::user() && $ownProfile)
            ...
            @else
            <x-profile-link href="{{ route('user-profile.posts', [$user->id]) }}"
                :active="request()->routeIs('user-profile.posts')">Posts</x-profile-link>
            <x-profile-link href="{{ route('user-profile.retweaks', [$user->id]) }}"
                :active="request()->routeIs('user-profile.retweaks')">Retweaks</x-profile-link>
            @endif
        </nav>
