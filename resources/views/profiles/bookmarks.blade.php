        ...
        <x-glass-container>
            ...
            <div class="flex align-center space-x-3 mb-4">
                <img class="w-12 h-12 rounded-full" src="https://flowbite.com/docs/images/people/profile-picture-5.jpg"
                    alt="user photo">
                <div>
                    <a href="#">{{ $bookmark->post->user->username }}</a>
                    <div class="flex items-center space-x-1">
                        @if ($bookmark->post->audience === 'public')
                        ...
                        @elseif ($bookmark->post->audience === 'friends')
                        ...
                        @elseif ($bookmark->post->audience === 'private')
                        ...
                        <p class="text-sm text-gray-300">{{ $bookmark->post->created_at->format('F d, Y') }}</p>
                    </div>
                </div>

                @can('update', $bookmark->post)
                <button id="dropdownMenuIconButton" data-dropdown-toggle="dropdownDots-{{ $bookmark->post->id }}"
                ...
                <!-- Dropdown menu -->
                <div id="dropdownDots-{{ $bookmark->post->id }}"
                    ...
                    <ul class="py-2 text-sm text-gray-700 dark:text-gray-200" aria-labelledby="dropdownMenuIconButton">
                        <li>
                            <a href="{{ route('posts.edit', [$bookmark->post->id]) }}"
            ...
            <div>
                <a href="{{ route('posts.show', [$bookmark->post->id]) }}">
                    <h1 class="text-2xl font-bold mb-1">{{ $bookmark->post->title }}</h1>
                    <p class="text-justify">{{ $bookmark->post->content }}</p>
                </a>
                <div>
                    @if($bookmark->post->attachments->isNotEmpty())
                    <div id="gallery" class="relative w-full" data-carousel="slide">
                        <!-- Carousel wrapper -->
                        <div class="relative h-56 overflow-hidden rounded-lg md:h-90">
                            @foreach($bookmark->post->attachments as $attachment)
                        ...
                        @if(count($bookmark->post->attachments) > 1)
                        <!-- Slider controls -->
                        <button type="button"
                    ...
                    @if ($bookmark->post->is_retweak == true)
                    <x-glass-container class="mb-3 mt-3">
                        <div class="flex align-center space-x-3 mb-4">
                            <img class="w-10 h-10 rounded-full" src="https://flowbite.com/docs/images/people/profile-picture-5.jpg"
                                alt="user photo">
                            <div>
                                <a href="#">{{ $bookmark->post->retweak->user->username }}</a>
                                <p class="text-sm text-gray-300">{{ $bookmark->post->retweak->created_at->format('F d, Y') }}</p>
                            </div>
                        </div>
                        <div class="mb-3">
                            <h1 class="text-2xl font-bold mb-1">{{ $bookmark->post->retweak->title }}</h1>
                            <p class="text-justify">{{ $bookmark->post->retweak->content }}</p>
                        </div>
                        <div class="grid {{ $bookmark->post->retweak->attachments->count() > 1 ? 'grid-cols-2 gap-2 mb-2' : 'grid-cols-1' }} ">
                            @foreach ($bookmark->post->retweak->attachments as $attachment)
                            <div>
            ...
            <div class="flex space-x-3">
                @php
                $hasLiked = $bookmark->post->likes->contains('user_id', Auth::user()->id);
                @endphp
                <button
                    ...
                    data-like-url="{{ route('likes.store', $bookmark->post->id) }}"
                    data-unlike-url="{{ route('likes.delete', $bookmark->post->id) }}"
                    ...
                    <span class="like-count" data-like-count="{{ $bookmark->post->likes->count() }}">{{ $bookmark->post->likes->count() }}</span>
                ...
                <a href="{{ route('posts.show', [$bookmark->post->id]) }}"
                    ...
                    <span>{{ $bookmark->post->comments->count() }}</span>
                    <span>Comment</span>
                </a>


                <a href="{{ route('retweaks.create', [$bookmark->post->id]) }}" class="flex space-x-1 py-1 px-2 rounded-xl transition duration-300 hover:bg-gray-500">
                    <svg class="w-6 h-6 text-gray-800 dark:text-white" aria-hidden="true"
                        xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24">
                        <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    <span>{{ $bookmark->post->retweaks->count() }}</span>
                    <span>Retweak</span>
                </a>


                @php
                $hasBookmarked = $bookmark->post->bookmarks->contains('user_id', Auth::user()->id);
                @endphp
                <button
                    ...
                    data-bookmark-url="{{ route('bookmarks.store', [$bookmark->post->id]) }}"
                    data-unbookmark-url="{{ route('bookmarks.delete', [$bookmark->post->id]) }}"
                    data-post-id="{{ $bookmark->post->id }}"
                    ...
                    <span class="bookmark-count">{{ $bookmark->post->bookmarks->count() }}</span>
