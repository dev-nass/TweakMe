        ...
        <x-glass-container>
            ...
            <div class="flex align-center space-x-3 mb-4">
                <img class="w-12 h-12 rounded-full" src="https://flowbite.com/docs/images/people/profile-picture-5.jpg"
                    alt="user photo">
                <div>
                    <a href="#">{{ $like->post->user->username }}</a>
                    <div class="flex items-center space-x-1">
                        @if ($like->post->audience === 'public')
                        ...
                        @elseif ($like->post->audience === 'friends')
                        ...
                        @elseif ($like->post->audience === 'private')
                        ...
                        <p class="text-sm text-gray-300">{{ $like->post->created_at->format('F d, Y') }}</p>
                    </div>
                </div>

                @can('update', $like->post)
                <button id="dropdownMenuIconButton" data-dropdown-toggle="dropdownDots-{{ $like->post->id }}"
                ...
                <!-- Dropdown menu -->
                <div id="dropdownDots-{{ $like->post->id }}"
                    ...
                    <ul class="py-2 text-sm text-gray-700 dark:text-gray-200" aria-labelledby="dropdownMenuIconButton">
                        <li>
                            <a href="{{ route('posts.edit', [$like->post->id]) }}"
            ...
            <div>
                <a href="{{ route('posts.show', [$like->post->id]) }}">
                    <h1 class="text-2xl font-bold mb-1">{{ $like->post->title }}</h1>
                    <p class="text-justify">{{ $like->post->content }}</p>
                </a>
                <div>
                    @if($like->post->attachments->isNotEmpty())
                    <div id="gallery" class="relative w-full" data-carousel="slide">
                        <!-- Carousel wrapper -->
                        <div class="relative h-56 overflow-hidden rounded-lg md:h-90">
                            @foreach($like->post->attachments as $attachment)
                        ...
                        @if(count($like->post->attachments) > 1)
                        <!-- Slider controls -->
                        <button type="button"
                    ...
                    <x-glass-container class="mb-3 mt-3">
                        <div class="flex align-center space-x-3 mb-4">
                            <img class="w-10 h-10 rounded-full" src="https://flowbite.com/docs/images/people/profile-picture-5.jpg"
                                alt="user photo">
                            <div>
                                <a href="#">{{ $like->post->retweak->user->username }}</a>
                                <p class="text-sm text-gray-300">{{ $like->post->retweak->created_at->format('F d, Y') }}</p>
                            </div>
                        </div>
                        <div class="mb-3">
                            <h1 class="text-2xl font-bold mb-1">{{ $like->post->retweak->title }}</h1>
                            <p class="text-justify">{{ $like->post->retweak->content }}</p>
                        </div>
                        <div class="grid {{ $like->post->retweak->attachments->count() > 1 ? 'grid-cols-2 gap-2 mb-2' : 'grid-cols-1' }} ">
                            @foreach ($like->post->retweak->attachments as $attachment)
                            <div>
            ...
            <div class="flex space-x-3">
                @php
                $hasLiked = $like->post->likes->contains('user_id', Auth::user()->id);
                @endphp
                <button
                    ...
                    data-like-url="{{ route('likes.store', $like->post->id) }}"
                    data-unlike-url="{{ route('likes.delete', $like->post->id) }}"
                    data-post-id="{{ $like->post->id }}"
                    ...
                    <span class="like-count" data-like-count="{{ $like->post->likes->count() }}">{{ $like->post->likes->count() }}</span>
                ...
                <a href="{{ route('posts.show', [$like->post->id]) }}"
                    ...
                    <span>{{ $like->post->comments->count() }}</span>
                    <span>Comment</span>
                </a>


                <a href="{{ route('retweaks.create', [$like->post->id]) }}" class="flex space-x-1 py-1 px-2 rounded-xl transition duration-300 hover:bg-gray-500">
                    <svg class="w-6 h-6 text-gray-800 dark:text-white" aria-hidden="true"
                        xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24">
                        <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    <span>{{ $like->post->retweaks->count() }}</span>
                    <span>Retweak</span>
                </a>


                @php
                $hasBookmarked = $like->post->bookmarks->contains('user_id', Auth::user()->id);
                @endphp
                <button
                    ...
                    data-bookmark-url="{{ route('bookmarks.store', [$like->post->id]) }}"
                    data-unbookmark-url="{{ route('bookmarks.delete', [$like->post->id]) }}"
                    data-post-id="{{ $like->post->id }}"
                    ...
                    <span class="bookmark-count">{{ $like->post->bookmarks->count() }}</span>
